...
class Adapter:
    ...
    def _interfaces_added(self, path, device_info):
        ...
        if constants.DEVICE_INTERFACE in device_info:
            ...
            SPP_UUID = '000018f0-0000-1000-8000-00805f9b34fb'
            if "Name" in device_info[dev_iface] and SPP_UUID in device_info[dev_iface].get('UUIDs'):
                device = {
                        'Name': device_info[dev_iface].get('Name') if 'Name' in device_info[dev_iface] else None,
                        'Alias': device_info[dev_iface].get('Alias') if 'Alias' in device_info[dev_iface] else None,
                        'Paired': device_info[dev_iface].get('Paired') if 'Paired' in device_info[dev_iface] else None,
                        'Connected': device_info[dev_iface].get('Connected') if 'Connected' in device_info[dev_iface] else None,
                        'Icon': device_info[dev_iface].get('Icon') if 'Icon' in device_info[dev_iface] else None,
                        'Address': device_info[dev_iface].get('Address') if 'Address' in device_info[dev_iface] else None,
                        'UUIDs': device_info[dev_iface].get('UUIDs') if 'UUIDs' in device_info[dev_iface] else None,
                    }
